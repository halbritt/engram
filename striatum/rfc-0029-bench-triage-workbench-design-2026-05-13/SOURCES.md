# Sources

Required context for this workflow:

- `AGENTS.md`
- `README.md`
- `HUMAN_REQUIREMENTS.md`
- `DECISION_LOG.md`
- `BUILD_PHASES.md`
- `ROADMAP.md`
- `SPEC.md`
- `docs/schema/README.md`
- `docs/rfcs/README.md`
- `docs/rfcs/0017-extraction-prompt-versioning.md`
- `docs/rfcs/0019-extraction-batching-server.md`
- `docs/rfcs/0024-phase-4-pre-full-corpus-benchmark-gate.md`
- `docs/rfcs/0027-interview-web-ui.md`
- `docs/specs/0027-interview-web-ui-spec.md`
- `docs/rfcs/0028-predicate-intent-surfacing.md`
- `docs/rfcs/0029-bench-triage-workbench.md`
- `docs/reviews/rfc0028-predicate-intent-implementation/REEXTRACTION_BENCH_100.md`
- `docs/process/multi-agent-review-loop.md`
- `docs/process/project-judgment.md`
- `benchmarks/extraction/`
- `src/engram/interview/web.py`

Private scratch artifacts may inform the local coordinator, but they must not be
quoted into tracked review artifacts.
