...
WORKFLOW=striatum/rfc-0027-interview-web-ui-review-rerun-2026-05-13/workflow.json
TARGET_REPO=.
```
...
"$RUNNER" --repo "$TARGET_REPO" branch confirm \
  --run-id "$RUN_ID" \
  --branch engram/rfc0027-interview-web-ui-review-rerun-2026-05-13 \
  --use-current --json
...
"$RUNNER" --repo "$TARGET_REPO" evidence export \
  --run-id "$RUN_ID" \
  --path docs/reviews/rfc0027-rerun-2026-05-13/EVIDENCE.md --json

"$RUNNER" --repo "$TARGET_REPO" run summary \
  --run-id "$RUN_ID" \
  --path docs/reviews/rfc0027-rerun-2026-05-13/RUN_SUMMARY.md --json
