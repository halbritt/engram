...
WORKFLOW=striatum/rfc-0028-predicate-intent-implementation/workflow.json
TARGET_REPO=.
```
...
"$RUNNER" --repo "$TARGET_REPO" evidence export \
  --run-id "$RUN_ID" \
  --path docs/reviews/rfc0028-predicate-intent-implementation/EVIDENCE.md --json
"$RUNNER" --repo "$TARGET_REPO" run summary \
  --run-id "$RUN_ID" \
  --path docs/reviews/rfc0028-predicate-intent-implementation/RUN_SUMMARY.md --json
make check-refs
```
