...
| D016 | accepted | Tiered Eval Structure (Smoke → Gold-Target → Full). | A 100-conversation random subset will not contain the entities queried by the gold set. | 100-conversation smoke test → 1,000-2,000 stratified gold-target subset → Full Corpus. The gold-target pass is the true gate. | Revisit if the stratified subset is too complex to construct. |
...
| D019 | accepted | Privacy Tiers and Contradictions Table in V1 schema. | Security principles and adversarial review require this schema for v1-to-v2 stability. | Add `privacy_tier` to captures/beliefs. Add `contradictions` table for conflict resolution. | LOW reversibility. Required infrastructure. |
